      ...
      for (auto &edit_op : args_.edit_ops) {
        switch (edit_op.command) {
          ...
          case Edit_Command::INSERT:
            ...
            break;
          case Edit_Command::WRITE_ACI:
            {
              xxxml::util::DF_Traverser t(doc);
              xfsx::tap::traverser::LXML_Proxy p(pretty_args.name_translator);
              xfsx::tap::traverser::Audit_Control_Info aci;
              aci(p, t);
              xxxml::util::remove(doc, "/TransferBatch/AuditControlInfo");
              xfsx::byte::writer::Memory m;
              aci.comment = "Generated by libxfsx";
              aci.print(m);
              xxxml::util::insert(doc, "/TransferBatch",
                  m.begin(), m.end(), -1);
            }
            break;
          default:
